...
function solveBoard(input) {
  ...
  const candidateNumbers = board.map((row, rowIndex) => row.map((value, colIndex) => value ? [] : candidates(board, rowIndex, colIndex)))
  const result = dlx.solveExactCover(board, { maxSolutions: 11 })
  const common = {
    count: result.count,
    countLabel: result.count >= 11 ? '>10' : String(result.count),
    countIsExact: result.count < 11,
    tooMany: result.count >= 11,
    givenCount,
    conflicts: [],
    candidates: candidateNumbers,
    metrics: { algorithm: 'DLX', nodesVisited: result.nodesVisited, elapsedMs: result.elapsedMs }
  }
  if (!result.count) return Object.assign({ status: 'none', solution: null, solutions: [] }, common)
  if (result.count === 1) return Object.assign({ status: 'unique', solution: result.solutions[0], solutions: result.solutions }, common)
  const listedSolutions = result.count >= 11 ? [] : result.solutions
  if (givenCount < 17) return Object.assign({ status: 'insufficient', solution: null, solutions: listedSolutions }, common)
  return Object.assign({ status: 'multiple', solution: null, solutions: listedSolutions }, common)
}
function countSolutions(input, limit) {
  if (!validate(input).valid) return 0
  return dlx.solveExactCover(input, { maxSolutions: Math.max(1, Number(limit) || 2) }).count
}
function generatePuzzle(difficulty) {
  const key = CONFIG[difficulty] ? difficulty : 'easy'
  const config = CONFIG[key]
  const generated = dlx.solveExactCover(emptyBoard(), { maxSolutions: 1, randomize: true })
  if (!generated.solutions.length) throw new Error('DLX_GENERATE_FAILED')
